...
function Scrubber({ offset, max, onChange, t }) {
    if (max <= 0)
        return null;
    return (_jsx("div", { className: css.scrubberWrap, children: _jsx("input", { type: "range", className: css.scrubber, min: 0, max: max, step: max >= 60_000 ? 1000 : 1, value: max - offset, "aria-label": t('scrubLabel'), onChange: (event) => { onChange(max - Number(event.currentTarget.value)); } }) }));
}
