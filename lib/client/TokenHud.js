...
function CollapsedChip({ total, onClick, t }) {
    return (_jsxs("button", { type: "button", className: css.chip, onClick: onClick, "aria-label": t('openPanel'), children: [_jsx("span", { className: css.chipDot, "aria-hidden": true }), _jsx("span", { className: css.chipLabel, children: "TOKEN" }), _jsx("span", { className: css.chipValue, children: formatNumber(total) })] }));
}
...
export function TokenHud({ t }) {
    ...
    if (snapshot === null) {
        return (_jsx("div", { className: css.host, children: _jsx(CollapsedChip, { total: 0, onClick: () => { setOpen(true); }, t: t }) }));
    }
    return (_jsxs("div", { className: css.host, children: [!open && _jsx(CollapsedChip, { total: totals.total, onClick: () => { setOpen(true); }, t: t }), open && (_jsxs("aside", { className: css.panel, "data-token-panel": true, children: [_jsxs("header", { className: css.head, children: [_jsxs("span", { className: css.title, children: [_jsx("span", { className: css.titleMark, "aria-hidden": true }), t('token')] }), _jsxs("div", { className: css.viewBar, role: "group", "aria-label": t('viewSwitch'), children: [_jsx("button", { type: "button", className: css.viewButton, "data-active": view === 'live', onClick: () => { setView('live'); }, children: t('live') }), _jsx("button", { type: "button", className: css.viewButton, "data-active": view === 'stats', onClick: () => { setView('stats'); }, children: t('stats') })] }), _jsx("button", { type: "button", className: css.closeButton, onClick: () => { setOpen(false); }, "aria-label": t('close'), children: "\u2715" })] }), view === 'live' ? (_jsxs(_Fragment, { children: [topHistory.length >= 2 && (_jsxs("div", { className: css.sparkWrap, children: [_jsx("div", { className: css.rangeBar, role: "group", "aria-label": t('timeRange'), children: RANGES.map((range) => (_jsx("button", { type: "button", className: css.rangeButton, "data-active": range.ms === rangeMs, onClick: () => { setRangeMs(range.ms); }, children: range.label }, range.label))) }), _jsx(Sparkline, { points: topHistory, now: now, t: t })] })), _jsxs("div", { className: css.body, children: [snapshot.sessions.length === 0 && (_jsx("span", { className: css.empty, children: t('noSessions') })), snapshot.sessions.slice(0, showAll ? undefined : 3).map((row) => (_jsx(SessionRow, { row: row, prices: prices, rangeMs: rangeMs, now: now, t: t }, row.sessionId))), !showAll && snapshot.sessions.length > 3 && (_jsx("button", { type: "button", className: css.moreButton, onClick: () => { setShowAll(true); }, children: fill(t('expandAll'), { count: snapshot.sessions.length }) })), showAll && snapshot.sessions.length > 3 && (_jsx("button", { type: "button", className: css.moreButton, onClick: () => { setShowAll(false); }, children: t('collapseAll') }))] })] })) : (_jsx("div", { className: css.body, children: _jsx(StatsView, { stats: stats, t: t }) })), _jsxs("footer", { className: css.foot, children: [_jsx("span", { className: css.footHint, children: error !== null
                                    ? fill(t('disconnected'), { error })
                                    : view === 'live'
                                        ? fill(t('pollLive'), { total: formatNumber(totals.total), out: formatNumber(totals.output) })
                                        : t('pollStats') }), _jsx("span", { className: css.mono, children: new Date(snapshot.generatedAt).toLocaleTimeString() })] })] }))] }));
}
