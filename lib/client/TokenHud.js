...
function StatsView({ stats, t, budgetMonthly, totalCost, effectiveBalance, editing, setEditing, editDraft, setEditDraft, editInputRef, onSave, onHint, onHintEnd }) {
    ...
    const renderEditor = (kind, value, display) => {
        ...
        return (_jsx("span", { className: css.editValue, onClick: () => { setEditDraft(String(value)); setEditing(kind); }, onPointerEnter: (event) => { onHint(t('editHint'), event.currentTarget, true); }, onPointerLeave: onHintEnd, children: display }));
    };
    const listCount = subView === 'months' ? stats.months.length : stats.days.length;
    const listItems = subView === 'months'
        ? stats.months.map((month) => (_jsx(StatBar, { label: monthLabel(month.month, t), value: month.total, max: maxMonth, input: month.input, output: month.output, cacheRead: month.cacheRead, cacheWrite: month.cacheWrite, cost: estimateCost(month.input, month.cacheRead, month.cacheWrite, month.output, prices) }, month.month)))
        : stats.days.map((day) => (_jsx(StatBar, { label: dateLabel(day.date, t), value: day.total, max: maxDay, input: day.input, output: day.output, cacheRead: day.cacheRead, cacheWrite: day.cacheWrite, cost: estimateCost(day.input, day.cacheRead, day.cacheWrite, day.output, prices) }, day.date)));
    return (_jsxs("div", { className: css.statsBody, children: [_jsxs("section", { className: css.section, children: [_jsx("div", { className: css.sectionLabel, children: _jsx("span", { children: t('summaryTitle') }) }), _jsxs("div", { className: css.statsTotal, children: [_jsx("span", { className: css.statsTotalLabel, children: t('totalLabel') }), _jsxs("div", { className: css.statsTotalRow, children: [_jsx("span", { className: css.mono, children: formatNumber(totalAll) }), _jsxs("span", { className: css.statsTotalSub, children: [t('totalSub'), " \u00B7 ", t('approx'), formatCost(totalCost)] })] })] }), budgetMonthly > 0 && (_jsxs("div", { className: css.editRow, children: [_jsx("span", { className: css.editLabel, children: t('budgetLabel') }), _jsx("span", { className: css.budgetTrack, children: _jsx("span", { className: css.budgetFill, style: { width: `${Math.min(100, (monthCost / budgetMonthly) * 100)}%` }, "data-over": monthCost > budgetMonthly }) }), renderEditor('budget', budgetMonthly, `¥${formatCost(budgetMonthly)}`)] })), effectiveBalance !== null && (_jsxs("div", { className: `${css.editRow} ${css.balanceEditRow}`, children: [_jsx("span", { className: css.editLabel, children: t('balanceLabel') }), renderEditor('balance', effectiveBalance, `¥${effectiveBalance.toFixed(2)}`)] })), _jsx("div", { className: css.editNote, children: t('balanceLocalNote') })] }), hasData && (_jsxs("section", { className: css.section, children: [_jsxs("div", { className: css.sectionLabel, children: [_jsx("span", { children: t('trendTitle') }), _jsxs("div", { className: css.viewBar, role: "group", "aria-label": t('granularity'), children: [_jsx("button", { type: "button", className: css.viewButton, "data-active": subView === 'days', onClick: () => { setListExpanded(false); setSubView('days'); }, children: t('byDay') }), _jsx("button", { type: "button", className: css.viewButton, "data-active": subView === 'months', onClick: () => { setListExpanded(false); setSubView('months'); }, children: t('byMonth') })] })] }), _jsx("div", { className: css.statsSparkWrap, children: subView === 'months'
                            ? (monthPoints.length >= 1 && (_jsx(Sparkline, { points: monthPoints, now: Date.now(), height: 64, tickFormat: (value) => formatMonthTick(value, t), t: t })))
                            : (dayPoints.length >= 1 && (_jsx(Sparkline, { points: dayPoints, now: Date.now(), height: 64, tickFormat: formatDateTick, t: t }))) })] })), hasData && listCount > 0 && (_jsxs("section", { className: css.section, children: [_jsxs("div", { className: css.sectionLabel, children: [_jsx("span", { children: subView === 'months' ? t('byMonth') : t('byDay') }), _jsx("span", { className: css.sectionCount, children: subView === 'months'
                                    ? fill(t('monthsCount'), { count: listCount })
                                    : fill(t('daysCount'), { count: listCount }) })] }), listExpanded && listItems, _jsxs("button", { type: "button", className: css.moreButton, onClick: () => { setListExpanded((current) => !current); }, children: [_jsx("span", { className: css.moreCaret, children: listExpanded ? '▴' : '▾' }), listExpanded
                                ? t('collapseList')
                                : subView === 'months'
...
export function TokenHud({ t, sessionsList }) {
    ...
    return (_jsxs("div", { ref: hostRef, className: css.host, style: hostStyle, "data-token-hud": true, children: [pressMenu && (_jsxs("div", { className: css.pressMenu, "data-press-menu": true, children: [_jsx("button", { type: "button", className: css.pressMenuItem, onClick: () => { goToDefault(); suppressClickUntilRef.current = 0; }, children: _jsxs("span", { className: css.pressMenuLabel, children: [_jsx(MenuIcon, { d: defaultPos === null || defaultPos.kind === 'corner'
                                    ...
                                    })()] })] })) : (_jsx("div", { className: css.body, children: _jsx(StatsView, { stats: stats, t: t, budgetMonthly: budgetMonthly, totalCost: totalCostNow, effectiveBalance: effectiveBalance, editing: editing, setEditing: setEditing, editDraft: editDraft, setEditDraft: setEditDraft, editInputRef: editInputRef, onSave: saveEdit, onHint: showHint, onHintEnd: hideHint }) })), _jsxs("footer", { className: css.foot, children: [_jsxs("span", { className: css.footHint, children: [error !== null
                                        ? fill(t('disconnected'), { error })
                                        : view === 'live'
                                            ? fill(t('pollLive'), { total: formatNumber(totals.total), out: formatNumber(totals.output) })
                                            : t('pollStats'), tps > 0 && (_jsxs("span", { className: css.footTps, children: [" \u00B7 ", tps >= 10 ? Math.round(tps) : tps.toFixed(1), " t/s"] })), prices.mode !== undefined && prices.mode !== 'flat' && (_jsx("span", { className: css.footPrice, "data-mode": prices.mode, children: prices.mode === 'peak' ? t('pricePeak') : t('priceOffpeak') }))] }), _jsxs("span", { className: css.footRight, children: [_jsxs("span", { className: css.footCost, children: [t('costNow'), " ", _jsxs("b", { children: ["\u00A5", totalCostNow.toFixed(2)] })] }), _jsx("span", { className: css.mono, children: new Date(snapshot.generatedAt).toLocaleTimeString() })] })] })] }))] }));
}
